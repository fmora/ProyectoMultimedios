...
public class ShowTravelConductor extends Activity{
    ...
    protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.show_travel_cond);
    ...
    class sea_travels extends AsyncTask<Void,Void,Boolean>{
		...
		protected Boolean doInBackground(Void... arg0) {
        	...
        	try {
				readIt(Viajes.searchTravelCondDB(user));
		...
		protected void onPostExecute(Boolean result){
			...
			if(ViajesDisponibles.size() == 0)
				Toast.makeText(getApplicationContext(),R.string.es_notravel, Toast.LENGTH_SHORT).show();
